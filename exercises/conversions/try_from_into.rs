...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        //if slice.get(0).unwrap() > &255 || slice.get(1).unwrap() > &255 || slice.get(2).unwrap() > &255 || slice.get(0).unwrap() < &0 || slice.get(1).unwrap() < &0 || slice.get(2).unwrap() < &0 {
        //    return Err(IntoColorError::IntConversion);
        //}

        //return Ok(Color{red: *slice.get(0).unwrap() as u8, green: *slice.get(1).unwrap() as u8, blue: *slice.get(2).unwrap() as u8})

        // the above works but below is a bit cleaner
